...
{
    public class NajizCaseLogService : INajizCaseLogService
    {
        private readonly INajizCaseLogRepository _repository;
        private readonly ICaseRepository _caseRepository;
        private readonly ICaseResearchersRepository _caseResearchersRepository;
        private readonly ILogger<NajizCaseLogService> _logger;

        public NajizCaseLogService(INajizCaseLogRepository repository, ICaseRepository caseRepository, ICaseResearchersRepository caseResearchersRepository, ILogger<NajizCaseLogService> logger)
        {
        ...
        public async Task<ReturnResult<NajizCaseDto>> AddAsync(NajizCaseDto model)

        {
            try
            {
                ...
                var caseDto = new CaseDto()
                {
                    ...
                    NajizRef = model.NajizRef,
                    NajizId = model.NajizId,
                    CaseNumberInSource = model.NajizCaseNo,
                    Subject = model.Subject,
                    LegalStatus = (MinistryLegalStatuses)(new Random().Next(1, 2)),
                ...
                return new ReturnResult<NajizCaseDto>(true, HttpStatuses.Status201Created, model);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, ex.Message, model);

                return new ReturnResult<NajizCaseDto>(false, HttpStatuses.Status500InternalServerError, new List<string> { "حدث خطأ أثناء تنفيذ العملية، يرجى المحاولة لاحقاً." });
            }
        }
    }
}
