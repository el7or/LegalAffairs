...
export class JDNConvertibleCalendarDateAdapter extends DateAdapter<JDNConvertibleCalendar> {
  ...
  constructor(@Optional() @Inject(MAT_DATE_LOCALE) dateLocale: string,
              @Inject(ACTIVE_CALENDAR) private activeCalendarToken:any) {
  ...
  getMonthNames(style: 'long' | 'short' | 'narrow'): any {
   ...
   */
  private createCalendarDate(year: number, month: number, date: number, calendar: string): any {
  ...
  parse(value: any, parseFormat: any): JDNConvertibleCalendar | null {
    ...
    if (parseFormat !== undefined
      ...
      switch (parseFormat) {
        case JDNConvertibleCalendarDateAdapter.DD_MM_YYYY: {
          ...
          if (parsed !== null) {
            ...
            console.error(`Error: parsing of date string failed: ${value}`);
            return null;
          }
        }
        default: {
          console.error(`Error: supported parsable format was not handled correctly: ${parseFormat}`);
          return null;
        }
      }
      ...
      console.error(`Error: unknown parseFormat ${parseFormat}`);
      return null;
    }
  ...
  format(date: JDNConvertibleCalendar, displayFormat: any): string {
    ...
    if (displayFormat !== undefined
      ...
      switch (displayFormat) {
        ...
        default: {
          console.error(`Error: supported display format was not handled correctly: ${displayFormat}`);
        }

      }

    } else {
      console.error(`Error: unknown displayFormat ${displayFormat}`);
    }
